...
function isMostlyUiChromeLine(line: string): boolean {
  const t = line.trim();
  if (!t) return true;
...
function extractLastAssistantAnswer(raw: string): string {
...
function paneLooksBusy(text: string): boolean {
  return /(Working|Thinking|Generating|Indexing|ctrl\+c to stop|[⠋⠙⠹⠸⠼⠴⠦⠧⠇⠏])/i.test(text);
}

function paneShowsInputPrompt(text: string): boolean {
